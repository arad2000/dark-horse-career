// frontend/app.js — نسخهٔ نهایی (V5.0)
// بارگذاری سوالات لایه دوم و سوم از data/questions.json
// ارسال پاسخ‌ها به API با ساختار استاندارد
...
const state = {
  ...
  selectedNarrowPaths: [],
  likedCodes: [],
  strategyAnswers: [],   // مقادیر عددی (۰ تا ۴)
  valueAnswers: [],      // کدهای "Q1A", "Q2B" و...
  currentQuestion: 0,
  swipeCards: [],
  swipeIndex: 0,
  totalSwipes: 0,
  // داده‌های سوالات (بارگذاری‌شده از فایل JSON)
  strategyQuestions: [],
  valueQuestions: []
};
...
function goBack() {
  ...
  state.stage = prev;
  // ریست کردن داده‌های مراحل بعدی
  if (prev === 'realm') {
    state.selectedSubRealms = []; state.selectedNarrowPaths = []; state.swipeCards = []; state.likedCodes = [];
  } else if (prev === 'subRealm') {
    state.selectedNarrowPaths = []; state.swipeCards = []; state.likedCodes = [];
  } else if (prev === 'narrowPath') {
    state.swipeCards = []; state.likedCodes = [];
  }
...
async function submitResults() {
  ...
  // پاسخ‌های ارزشی به صورت دیکشنری
...
// ==================== INIT ====================
async function init() {
  await loadQuestionData(); // بارگذاری سوالات قبل از هر چیز
  render();
}
