// frontend/app.js — نسخهٔ نهایی (V25.0)
// شامل: Manifesto, Guide, Splash, سه حلقه, سوایپ, لایه دوم و سوم, تحلیل سبک,
// سناریوهای همسویی, نظرسنجی ۱۰ سوالی, دکمهٔ مشاهده بازخوردها, ذخیره خودکار
  ...
  app.innerHTML = `
    <div style="text-align:right;padding:10px;">
      ...
      <div class="card" style="margin-top:15px;">
        ...
        <p style="color:#b0a080;line-height:2.2;">۱. <strong>فضایی آرام انتخاب کن.</strong> جایی که سکوت باشد و کسی مزاحمت نشود. هدفون بگذار و گوشی را در حالت «مزاحم نشوید» قرار بده.<br>
...
// ==================== DISPLAY RESULTS (V23 با سناریوهای هوشمند و نظرسنجی) ====================
      ...
      if (r.evidence && typeof r.evidence === 'object') {
        ...
        if (hasStrategy && hasValue && isMicroWeak && bestMajor && r.major_id !== bestMajor.major_id) {
          ...
          if (strategyStyle && valueStyle) {
            ...
            if (strategyDomain) suggestionText += ` بر اساس پاسخ‌هایت، سبک فکری «${strategyStyle.style}» در تو قوی‌تر است. شاید علاقه‌مند باشی حوزه‌های ${strategyDomain} را نیز بررسی کنی.`;
          } else if (valueStyle) {
            const valueDomain = getDomainFromValues(valueStyle.summary).slice(0, 3).join('، ');
            if (valueDomain) suggestionText += ` بر اساس پاسخ‌هایت، ارزش‌های «${valueStyle.summary}» در تو قوی‌تر است. شاید علاقه‌مند باشی حوزه‌های ${valueDomain} را نیز بررسی کنی.`;
          }
...
function submitFeedback() {
  ...
  console.log('📋 بازخورد ثبت شد:', allFeedback);
}
...
function copyAllFeedback() {
  ...
  navigator.clipboard.writeText(text).then(() => {
    alert('✅ تمام بازخوردها به صورت JSON کپی شد. می‌توانی در یک فایل ذخیره کنی یا برای من بفرستی.');
  }).catch(() => {
