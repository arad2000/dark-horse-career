...
// ==================== DISPLAY RESULTS (با دیباگ پیشرفته) ====================
function displayResults(data, sjt, conj) {
  ...
  // بررسی قابلیت‌های API
  const hasDetailedScores = matched.length > 0 && matched[0].m_score !== undefined;
  const hasEvidence = matched.length > 0 && matched[0].evidence !== undefined;
  const hasCommonMotives = matched.length > 0 && matched[0].common_micro_motives !== undefined;
  ...
  let html = `
    ...
    <div style="background:#1a1a2e;border:1px solid #d4af37;border-radius:12px;padding:12px;margin:15px 0;text-align:right;font-size:0.8rem;color:#f0c040;">
      <p style="margin:0;font-weight:bold;">📤 اطلاعات ارسالی به موتور اسب سیاه:</p>
    ...
    <div style="background:#1a1a2e;border-radius:12px;padding:12px;margin:15px 0;text-align:right;font-size:0.85rem;color:#aaa;">
      ...
      ${!hasDetailedScores ? '<p style="color:#f0c040;margin:5px 0;">⚠️ API در حال حاضر امتیاز تفکیکی (m_score, s_score, v_score) را ارائه نمی‌دهد. امتیاز نهایی ترکیبی از هر سه لایه است.</p>' : ''}
    </div>`;
