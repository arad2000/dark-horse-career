...
class DarkHorseMatcher:
    ...
    def __init__(self, data_dir: str = "data"):
        ...
        self.strategy_questions: List[Dict] = []
        ...
        logger.info(f"   ❓ {len(self.strategy_questions)} strategy questions loaded")
        logger.info(f"   ⚖️  {len(self.value_questions)} value questions loaded")
    ...
    def _load_questions(self):
        """بارگذاری هر دو دسته سوالات: استراتژی و ارزشی"""
        ...
        try:
            ...
            # بارگذاری سوالات استراتژی
            self.strategy_questions = layers.get("strategies", {}).get("questions", [])
            
            # بارگذاری سوالات ارزشی (ترجیحی)
            self.value_questions = layers.get("values", {}).get("questions", [])
    ...
    def _extract_traits(self, strategy_answers: List[int]) -> Set[str]:
        ...
        for i, ans_idx in enumerate(strategy_answers):
            if 0 <= i < len(self.strategy_questions):
                question = self.strategy_questions[i]
                options = question.get("options", [])
                if 0 <= ans_idx < len(options):
                    opt = options[ans_idx]
                    trait = opt.get("trait_hint", "") if isinstance(opt, dict) else ""
                    if trait:
                        traits.add(trait)
        return traits
    ...
    def _calculate_penalty(self, reality_check: Optional[Dict], major: Dict) -> float:
        ...
        Args:
            reality_check: دیکشنری شامل pressure_tolerance, prestige_sensitivity, financial_constraint
            major: رشته در حال بررسی
        
        Returns:
            penalty بین ۰ و MAX_PENALTY
        """
        ...
        # جریمه فشار (برای رشته‌های پرفشار مثل پزشکی، اورژانس)
        ...
        # جریمه پرستیژ (برای رشته‌های کم‌پرستیژ)
        ...
        # جریمه مالی (برای رشته‌های با درآمد پایین)
    ...
    def _generate_explanation(self, user_motives, strategy_answers, value_choices, major,
    matched_motives, m_score, s_score, v_score):
        parts = []
        if matched_motives:
    ...
    def match(self, user_motives: List[str], strategy_answers: List[int],
        ...
        return {
            ...
            "nonlinear_pathways": pathways,            "summary": {
...
if __name__ == "__main__":
    print("🐎 Dark Horse Engine v6.1 - Test")
