...
if not DATA_DIR.exists():    logger.error(f"❌ Data directory not found: {DATA_DIR}")
...
class MatchRequest(BaseModel):
    micro_motives: List[str] = Field(..., min_items=1, max_items=50)
    strategies: List[int] = Field(..., min_items=20, max_items=20)
    values: List[str] = Field(..., min_items=10, max_items=10)
...
def root():
    ...
    return {
        ...
        "data_loaded": {
            ...
            "micro_motives": len(matcher.micro_motives),            "strategy_questions": len(matcher.strategy_questions),
...
@app.get("/majors")def list_majors():
    """لیست تمام ۱۵۰ رشته"""
    majors_list = [
        {
            "id": m["id"],
...
def get_value_questions():
    """دریافت ۱۰ سوال ارزشی (ترجیحی)"""
    return {
        "total": len(matcher.value_questions),
