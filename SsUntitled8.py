"""
Sanjesh Engine v9.1 - Final Revised
اصلاحات:
- اضافه شدن nobat_dovom به ضرایب دوره
- سازگاری کامل با ساختار programs_final_v5.2.json
- بهبود استخراج کات‌آف‌ها
"""
import logging
...
COURSE_TYPE_MULTIPLIER = {
    "roozaneh": 1.00,
    "shabaneh": 1.15,
    "nobat_dovom": 1.15,   # اضافه‌شده برای دوره‌های نوبت دوم
    "pardis": 1.40,
    "majazi": 1.50,
    "azad": 1.60,
    "payam_noor": 2.00,
...
def resolve_major_id(major_id: str, programs_database: List[Dict]) -> List[str]:
    """تبدیل major_id به فرمت‌های مختلف (عددی و انگلیسی)"""
    ...
    # بررسی اینکه کدام در programs وجود دارد
    for prog in programs_database[:20]:  # بررسی 20 برنامه اول
...
def extract_cutoff_from_structure(cutoff_structure: Any, target_year: str = "1404") -> Dict[str, Any]:
    """
    استخراج cutoff از ساختار تو در تو
    اگر cutoff یک dict تو در تو است (شامل سال‌ها)، سال مورد نظر را استخراج می‌کند
    اگر یک dict ساده است، مستقیماً برمی‌گرداند
    """
    if not isinstance(cutoff_structure, dict):
        return {}
    
    # بررسی اینکه آیا این یک ساختار تو در تو است (شامل سال‌ها)
    if "1401" in cutoff_structure or "1402" in cutoff_structure or "1403" in cutoff_structure or "1404" in cutoff_structure:
        # ساختار تو در تو - سال مورد نظر را استخراج کن
        year_cutoff = cutoff_structure.get(target_year, {})
        if isinstance(year_cutoff, dict):
            return year_cutoff
        return {}
    
    # ساختار ساده - مستقیماً برگردان
...
def calculate_effective_cutoff(
    ...
    """محاسبه cutoff موثر با در نظر گرفتن بومی‌گزینی و سهمیه"""
    warnings = []
    ...
    # انتخاب cutoff مناسب (بومی یا غیربومی)
    if is_bomi:
    ...
    # استخراج cutoff از ساختار تو در تو (سال ۱۴۰۴)
    cutoff_dict = extract_cutoff_from_structure(raw_cutoffs, "1404")
    
    if not cutoff_dict:
        # اگر cutoff_dict خالی است، از cutoffs_historical استفاده کن
    ...
    if user_quota in ("isargaran_25", "shahid"):
        ...
        if free_cutoff > 0:
            ...
            warnings.append(f"✓ قانون ۷۰٪ ایثارگران اعمال شد")
        else:
        ...
        if free_cutoff > 0:
            ...
            warnings.append(f"✓ قانون ۸۵٪ ایثارگران اعمال شد")
        else:
            cutoff = cutoff_dict.get(user_quota, 0)
    else:
        cutoff = cutoff_dict.get(user_quota, 0)
    ...
    # اعمال ضریب نوع دوره (مثلاً نوبت دوم)
...
def apply_gpa_impact(user: Dict[str, Any], program: Dict[str, Any]) -> float:
    """
    اعمال تاثیر معدل بر رتبه (MGP Factor)
    هر نمره معدل = ۴٪ تغییر در رتبه موثر
    """
    try:
...
def calculate_weighted_historical_cutoffs(cutoffs_historical: Dict) -> Dict:
    """محاسبه میانگین وزن‌دار cutoff های تاریخی"""
    cutoffs = {}
...
def calculate_advanced_exam_probability(user: Dict, program: Dict) -> Tuple[float, List[str]]:
    """محاسبه احتمال قبولی در پذیرش با آزمون"""
    try:
        all_warnings = []
        ...
        # انتخاب cutoff (اولویت با پیش‌بینی ۱۴۰۵، سپس تاریخی، سپس قدیمی)
        ...
        # محاسبه cutoff موثر (شامل بومی‌گزینی، سهمیه، نوع دوره)
...
def calculate_savabegh_probability(user: Dict, program: Dict) -> Tuple[float, List[str]]:
    """محاسبه احتمال قبولی در پذیرش سوابق تحصیلی"""
    try:
        all_warnings = []
...
def get_eligible_universities(
    ...
    """دریافت لیست دانشگاه‌های قابل قبول"""
    eligible = []
...
def calculate_admission_for_major(
    ...
    """محاسبه شانس قبولی برای یک رشته خاص"""
...
def calculate_all_majors_admission(
    ...
    """محاسبه شانس قبولی برای همه رشته‌ها"""
    all_results = []
    ...
    # مرتب‌سازی بر اساس حداکثر احتمال
