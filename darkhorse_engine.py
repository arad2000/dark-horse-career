"""
Dark Horse Engine v9.1 - سازگار با داده‌های جدید
==================================================
سازگار با:
  - ۱۵۰ رشته در ۶ قلمرو
  - ۹۶۰ میکروموتیو کدگذاری‌شده (micro_motive_codes)
  - ماتریس ۲۰×۵ استراتژی (strategy_weights)
  - ۱۰ کد ارزشی (value_weights: Q1A-Q10B)
  - main.py v9.0 (ساختار خروجی مشابه v5.2.3)

ورودی‌ها (از main.py):
  - majors_database: dict با کلید major_id
  - user_motives: List[str] کدهای انتخابی
  - sjt_answers: Dict[str, str] مانند {"sjt_1": "A", ...}
  - conjoint_choices: Dict[str, str] مانند {"conj_1": "A", ...}
...
MISFIT_TRAITS: Set[str] = {
    ...
    "فشارپذیری", "انعطاف‌پذیری", "مداخله‌گر", "آرام",
}
...
# =========================================================================
# Cache برای نگاشت کد → قلمرو
# =========================================================================

class RealmCache:
    """Cache برای نگاشت کد میکروموتیو به قلمرو (O(1) lookup)"""
    
    def __init__(self, majors_database: Dict):
        self.code_to_realm: Dict[str, str] = {}
        self._build(majors_database)
    
    def _build(self, majors_database: Dict) -> None:
...
def _load_questions_data() -> Tuple[List[Dict], List[Dict]]:
    """بارگذاری questions.json برای استخراج trait_hint"""
    try:
        ...
        for path in possible_paths:
            if path.exists():
                ...
                strategy_qs = layers.get("strategies", {}).get("questions", [])
                value_qs = layers.get("values", {}).get("questions", [])
                return strategy_qs, value_qs
    except Exception as e:
        logger.warning(f"Could not load questions.json: {e}")
    return [], []


def extract_traits_from_sjt(sjt_answers: Dict[str, str]) -> Set[str]:
    """استخراج trait از پاسخ‌های SJT"""
    traits: Set[str] = set()
    if not sjt_answers:
        return traits
    
    strategy_qs, _ = _load_questions_data()
    
    for i in range(1, 21):
        key = f"sjt_{i}"
        if key not in sjt_answers:
            continue
        
        answer = sjt_answers[key].strip().upper()
        idx = ord(answer) - ord('A') if len(answer) == 1 and answer.isalpha() else 0
        
        if 0 <= (i - 1) < len(strategy_qs):
            question = strategy_qs[i - 1]
...
def constructive_misfit_score(traits: Set[str]) -> float:
    """CMS سه‌گانه: Misfit/Neutral/Linear"""
    if not traits:
        return 0.40
...
def micro_motives_match(
    ...
    """
    تطبیق میکروموتیوها با F1 هوشمند
    Returns: (score بین ۰ و ۱, لیست کدهای مشترک)
    """
...
def sjt_environment_score(
    ...
    """میانگین وزن‌های انتخاب‌شده از ماتریس ۲۰×۵"""
...
def calculate_value_score(
    ...
    """میانگین وزن‌های ارزش‌های انتخاب‌شده"""
    ...
    for choice in value_choices:
        if choice and choice.strip():
            ...
            if choice_clean in value_weights:
                total += value_weights[choice_clean]
                valid += 1
...
def calculate_pathway_bonus(
    ...
    """پاداش مسیر غیرخطی با استفاده از cache"""
    if not user_motives:
        return 0.0
...
def calculate_penalties(
    ...
    """جریمه بر اساس واقعیت‌های کاربر"""
    ...
    pressure = float(reality_check.get("pressure_tolerance", 3))
    if realm in {"healer", "builder"} and pressure < 3.0:
        penalty += min(0.12, 0.15 * (3.0 - pressure) / 4.0)
    
    prestige = float(reality_check.get("prestige_sensitivity", 3))
    if realm == "communicator" and prestige > 3.5:
        penalty += min(0.08, 0.08 * (prestige - 3) / 4.0)
    
    financial = float(reality_check.get("financial_constraint", 3))
    if realm in {"explorer", "artist", "thinker"} and financial > 3.5:
        penalty += min(0.10, 0.10 * (financial - 3) / 4.0)
...
def calculate_major_fit(
    ...
    # Bonuses
    bonus = calculate_pathway_bonus(major_data, user_motives, realm_cache, cms)
    ...
    return {
        ...
        "individuality_fit": {
            ...
            "evidence": {
                ...
                "traits_matched": list(user_traits),
...
def discover_individuality(
    ...
    """
    کشف فردیت - سازگار با main.py v9.0
    """
    # تبدیل sjt_answers به strategy_answers (list ایندکس)
    strategy_answers: List[int] = []
    for i in range(1, 21):
        key = f"sjt_{i}"
    ...
    # محاسبه CMS و وزن‌ها
    user_traits = extract_traits_from_sjt(sjt_answers)
    ...
    logger.info(f"📊 Input: {len(user_motives)} motives, {len(user_traits)} traits")
    
    # ساخت cache
    realm_cache = RealmCache(majors_database)
    
    # محاسبه برای همه رشته‌ها
    all_fits: List[Dict] = []
    failed_majors: List[str] = []
    
    for major_id, major_data in majors_database.items():
        try:
            fit = calculate_major_fit(
                major_id=major_id,
                ...
                cms=cms,
                w1=w1, w2=w2, w3=w3,
                realm_cache=realm_cache,
                reality_check=reality_check,
                user_traits=user_traits,
            )
            all_fits.append(fit)
        except Exception as e:
            logger.error(f"Error major {major_id}: {e}", exc_info=True)
            failed_majors.append(major_id)
    ...
    return {
        ...
        "method": {
            "principle": (
                ...
                "۲۰ سوال استراتژی، ۱۰ سوال ارزشی فلسفی "
                "و تعدیل واقعیت‌ها"
            ),
